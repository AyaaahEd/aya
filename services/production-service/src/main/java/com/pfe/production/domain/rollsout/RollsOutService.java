...
import com.pfe.production.domain.job.JobService;

import com.pfe.production.domain.rollsout.RollsOut.RollsOutState;
...
public class RollsOutService {
    ...
    public RollsOut updateState(String id, RollsOutState newState) {
        ...
        if (newState == RollsOutState.FINISHED) {
            List<Job> jobs = jobRepository.findByFormVersionId(roll.getFormVersionId());
            for (Job job : jobs) {
