...
public class Machine {
    ...
    /**
     * Checks if this machine is available during the given time range.
     */
    public boolean isAvailableDuring(LocalDateTime start, LocalDateTime end) {
        if (reservations == null || reservations.isEmpty())
            return true;
        return reservations.stream()
                .noneMatch(r -> r.getStartDate().isBefore(end) && r.getEndDate().isAfter(start));
    }

    /**
     * Adds a reservation to this machine after validating no overlap.
     * 
     * @throws IllegalStateException if reservation overlaps with existing ones
     */
    public void addReservation(Reservation reservation) {
        if (reservations == null) {
            reservations = new java.util.ArrayList<>();
        }
        if (!isAvailableDuring(reservation.getStartDate(), reservation.getEndDate())) {
            throw new IllegalStateException("Reservation overlaps with existing reservation on machine: " + name);
        }
    ...
    /**
     * Gets the speed configuration for a given quality code.
     * 
     * @throws IllegalArgumentException if no matching speed found
     */
    public Speed getSpeedForQuality(String qualityCode) {
        if (speeds == null || speeds.isEmpty()) {
            throw new IllegalArgumentException("No speed configurations defined for machine: " + name);
        }
        return speeds.stream()
                .filter(s -> qualityCode.equals(s.getQualityCode()))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException(
                        "No speed configuration found for quality code: " + qualityCode + " on machine: " + name));
    }
